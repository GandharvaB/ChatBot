...
export function ControlBar() {
  ...
  const { startRecording, stopRecording, stopPlayback, processFinalTranscript } = useSpeech();

  const handleTextSubmit = async (e) => {
    e.preventDefault();
    if (!textInput.trim()) return;

    // Link manual text input to the AI response pipeline
    processFinalTranscript(textInput.trim(), settings.language || 'en-US');
  ...
  const isWakeWordMode = avatarStore.isWakeWordMode;
  const error = avatarStore.error;
  const showTimeoutWarning = avatarStore.showTimeoutWarning;
  const sessionEndTime = avatarStore.sessionEndTime;

  const [timeLeft, setTimeLeft] = React.useState(0);

  React.useEffect(() => {
    if (!showTimeoutWarning || !sessionEndTime) return;
  ...
  }, [showTimeoutWarning, sessionEndTime]);
  ...
  return (
    <div className="flex flex-col relative w-full mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {showTextInput && (
        <form onSubmit={handleTextSubmit} className="absolute bottom-18 left-0 right-0 p-2 bg-white/10 backdrop-blur-xl rounded-2xl border border-black/20 z-50 shadow-2xl flex gap-2">
          <input
            ...
            placeholder="Message..."
            className="flex-1 bg-black/40 text-white px-4 py-2 rounded-xl outline-none text-sm border border-black/5 active:border-black/20 transition-all"
            autoFocus
          />
      ...
      {showTimeoutWarning && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-max px-4 py-2 bg-amber-500/90 text-white text-xs font-bold rounded-full animate-bounce shadow-lg backdrop-blur-md border border-white/20">
          Still there? Session ends in {timeLeft}s...
        </div>
      )}

      {/* Error Message */}
      {error && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-max px-4 py-2 bg-red-500/90 text-white text-xs font-bold rounded-full shadow-lg backdrop-blur-md border border-white/20">
          {error}
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-6 w-full h-16 px-4 relative overflow-hidden">
        ...
        {(isProcessing || avatarState === AVATAR_STATES.SPEAKING) && <div className="absolute inset-0 bg-blue-500/5 animate-pulse pointer-events-none" />}
        ...
        <div className="relative flex items-center justify-center">
          {!isRecording && !isProcessing ? (
            <button
              ...
              title={isWakeWordMode ? "Hold to Talk" : "Processing"}
            >
        ...
        <div className="flex flex-col items-start gap-0.5">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${isWakeWordMode ? 'bg-blue-400 animate-pulse' : (avatarStore.currentTranscript ? 'bg-green-400 animate-bounce' : 'bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.6)]')}`} />
            <span className="text-[10px] font-bold uppercase tracking-wider text-white/70">
              {isWakeWordMode ? 'Ready for Command' : (avatarState === AVATAR_STATES.SPEAKING ? 'Belli is Speaking' : (avatarState === AVATAR_STATES.THINKING ? 'Belli is Thinking' : (avatarStore.currentTranscript ? 'Listening...' : 'Ready?')))}
            </span>
          </div>
          {isWakeWordMode && (
            <span className="text-[9px] text-white/40 italic">Say "Hi Belli" to start</span>
          )}
          {!isWakeWordMode && avatarStore.currentTranscript && !isProcessing && avatarState !== AVATAR_STATES.SPEAKING && (
            <span className="text-[9px] text-green-400/80 italic font-medium truncate max-w-[120px]">
              "{avatarStore.currentTranscript}..."
            </span>
          )}
        </div>
